...
# Ensure composer exists early
if ! command -v composer >/dev/null 2>&1; then
  echo "composer not found in PATH; please install composer or add it to PATH" >&2
  exit 2
fi
...
# Helper to log with timestamp (writes to console and report)
log() {
  local ts msg
...
log "Step 3: composer validate (no network, non-interactive)"
# Run composer validate non-interactively and allow superuser for this invocation; stream output to console and report
COMPOSER_ALLOW_SUPERUSER=1 COMPOSER_NON_INTERACTIVE=1 \
  composer validate --no-check-publish --strict --no-plugins --no-scripts 2>&1 | tee -a "$REPORT" || log "composer validate returned non-zero (see report)"
log "Finished Step 3"

if [ "$BRIEF" -eq 0 ]; then
  log "Step 4: composer dump-autoload --optimize (this may be slow)"
  # Run without blocking output and capture exit code; stream to report
  if COMPOSER_ALLOW_SUPERUSER=1 COMPOSER_NON_INTERACTIVE=1 composer dump-autoload --optimize --no-dev 2>&1 | tee -a "$REPORT"; then
...
grep -R --line-number "^namespace " unified KCM app 2>/dev/null | sed -n '1,200p' > "$NAMESPACES" || true
