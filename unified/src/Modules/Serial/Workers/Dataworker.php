...
class Dataworker
{
    ...
    public function getSerialAvgs()
    {
        ...
        foreach ($ifacesRaw as $iface) {
            ...
            $serialArray[$ifaceName]['hour'] = $this->getSerialData($ifaceId, "-1 hour");
            $serialArray[$ifaceName]['thirty'] = $this->getSerialData($ifaceId, "-30 minutes");
            $serialArray[$ifaceName]['ten'] = $this->getSerialData($ifaceId, "-10 minutes");
    ...
    public function getSerialData($iface, $timeVal){
        ...
        if (empty($results) || !isset($results[0])) {
            // Return 0 if no serial settings found for this interface
            return 0;
        }
    ...
    {
        $sql = "select MAX(epoch) as max_epoch from serial_data WHERE iface = ? AND size > 0";
