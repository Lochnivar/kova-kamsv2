...
class Udp
{
    ...
    public function udpSum()
    {
        ...
        if (empty($ifaces)) {
            ...
            return '<div style="text-align: center; padding: 20px; color: #666;">No UDP interfaces configured</div>';
        }
        ...
        foreach ($ifaces as $iface) {
            ...
            $cargo .= <<<EOL
                        <div class="gauge-item">
                            <canvas id = 'gauge-udp-{$ifaceName}' class='gauge' width='175' height = '128'></canvas>
                            <div class="gauge-label">{$label}</div>
                            <div class="gauge-timestamp" id = 'lastpacket-udp-{$ifaceName}'></div>
                        </div>
        EOL;
        }
